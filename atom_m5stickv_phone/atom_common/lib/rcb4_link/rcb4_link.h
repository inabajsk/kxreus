...
/// ATOM's external I2C bus -- see M5STICKV_OPCODE's own comment for the
/// wire protocol, unchanged from atom/s3_echo_with_I2C's own
/// atoms3_i2c_robot.ino, which established both this opcode and the pin
/// move below.
class Rcb4Link {
public:
    ...
    ///
    /// G6(TX)/G5(RX) rather than the Grove connector's G2/G1: those solder
    /// pads on the board's underside are independent of the Grove
    /// connector, which this firmware needs free for the M5StickV's I2C
    /// (see M5STICKV_SDA_PIN/M5STICKV_SCL_PIN) -- and independent of
    /// G38/G39 (the ATOM's own internal IMU), so all three work at once.
    /// Matches atom/s3_echo_with_I2C's own atoms3_i2c_robot.ino exactly,
    /// which moved this first for the same reason.
    ///
    /// M5Stack FIRE builds (ARDUINO_M5STACK_FIRE, set by platformio's
    /// `m5stack-fire` board) use Port B -- G26(TX)/G36(RX) -- NOT Port C
    /// (G17/G16), despite Port C being the board's own silkscreened
    /// "serial" port and the pins UART2 defaults to. Confirmed against
    /// M5Stack's own docs (m5-docs core/fire.md) and reproduced on real
    /// hardware (2026.9): G16/G17 on FIRE specifically are also wired to
    /// this board's PSRAM chip, and using them for anything else -- even
    /// just this link's own gpio_reset_pin()+UART setup in begin() --
    /// leaves Port C's UART unable to reach a real RCB-4 at all (measured:
    /// zero bytes back from a probe, on wiring otherwise confirmed correct)
    /// AND is the likely cause of this same board's PSRAM boot-time
    /// self-test failures logged earlier the same session ("spiram: SPI
    /// SRAM memory test fail") -- a second, previously-unexplained symptom
    /// this one wiring mistake plausibly accounts for on its own. M5Stack's
    /// own docs name Port B as the fallback once Port C is ruled out this
    /// way. G36 is one of this chip's input-only pins (GPIO34-39), which is
    /// fine for RX and is why it, not G26, is the RX side.
    ///
    /// There is no free solder-pad UART on this board the way there is on
    /// AtomS3, and UART0 (G1/G3) is Serial, carried out over the CP2104 USB
    /// bridge chip, not a native USB CDC port. Port A (G21/G22) is left for
    /// the M5StickV, same as G1/G2 was on AtomS3 -- except THERE it is also
    /// the bus the board's own internal IMU/RTC are wired to; see
    /// M5STICKV_SDA_PIN's own comment and beginM5StickV().
...
#else
    static constexpr int TX_PIN = 6;  ///< ATOM TX (G6, solder pad) -> RCB-4 Rx
    static constexpr int RX_PIN = 5;  ///< ATOM RX (G5, solder pad) <- RCB-4 Tx
    static constexpr int UART_PORT_NUM = 1;
#endif
    ...
    /// External Grove (HY2.0-4P) connector, freed by the TX_PIN/RX_PIN move
    /// above. Bare AtomS3's board definition (M5Unified) has SCL=G1/SDA=G2.
    ///
    /// M5Stack FIRE: Port A, G21(SDA)/G22(SCL) -- but unlike AtomS3, this is
    /// NOT a bus of its own: M5Unified's _setup_i2c() gives this board the
    /// same SDA/SCL for its internal bus (the MPU6886 IMU, the BM8563 RTC)
    /// as for Port A, so M5.begin() has already brought this exact bus up
    /// before beginM5StickV() runs. See that function's own comment for why
    /// it therefore does not call Wire.begin() here, and the .cpp's i2c*()
    /// helpers for why the M5StickV calls go through M5.Ex_I2C instead of
    /// Wire on this board -- a second, independent driver on pins the one
    /// M5Unified already installed would only fight it.
#if defined(ARDUINO_M5STACK_FIRE)
    static constexpr int M5STICKV_SDA_PIN = 21;  ///< Port A (G21)
    static constexpr int M5STICKV_SCL_PIN = 22;  ///< Port A (G22)
#else
    static constexpr int M5STICKV_SDA_PIN = 2;  ///< G2
    static constexpr int M5STICKV_SCL_PIN = 1;  ///< G1
#endif
    ...
    /// Brings up the M5StickV's I2C bus (Wire, on M5STICKV_SDA_PIN/
    /// M5STICKV_SCL_PIN). Separate from begin() -- a different bus
    /// entirely -- and from M5.Imu's own I2C, which M5Unified keeps on its
    /// own internal bus (G38/G39) rather than the global Wire this uses.

...
//
// Bound to kxrl2gwalk rather than an arbitrary actor because every actor
// compiled into THIS build (kxrl2g's only actor: walk) shares one obs/action
// layout and control rate -- they are the same robot's two skills, not two
// different robots the way crawl/omni/etc were.
#define POLICY_OBS_DIM POLICY_KXRL2GWALK_OBS_DIM
#define POLICY_ACT_DIM POLICY_KXRL2GWALK_ACT_DIM
...
#define POLICY_OBS_JOINT_VEL_START POLICY_KXRL2GWALK_OBS_JOINT_VEL_START
#define POLICY_OBS_ACTIONS_START POLICY_KXRL2GWALK_OBS_ACTIONS_START

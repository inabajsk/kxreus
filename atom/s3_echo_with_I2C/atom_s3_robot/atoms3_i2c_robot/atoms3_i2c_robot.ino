// ロボット側 ATOM S3 用ファームウェア(I2C拡張版)。
//
// atoms3_voice_ei_robot.ino をベースに、Grove(HY2.0-4P)コネクタのG1/G2を
// 外部I2C用に空けるため、RCB4 UARTをG5/G6(基板底面の拡張用GPIOパッド、
// Groveコネクタではなくはんだ付けで配線)に付け替えたバリエーション。
// G38/G39は内蔵IMU(MPU6886)用のままなので、IMU予約OPCODEの仕組みは
// atoms3_voice_ei_robot.ino と同じく使える。
// 今後、G1/G2に外部I2Cデバイス(センサ、M5Cameraなど)を追加していく
// 拡張のベースとして、このフォルダで発展させていく。
//
// 【ピン配置(ATOM S3、このバリエーション)】
//   RCB4 UART: G6(TX)/G5(RX) -- 基板底面の拡張用GPIOパッド(はんだ付け)。
//     G1/G2(Groveコネクタ)・G38/G39(内蔵IMU)とは独立しているため、
//     3つとも同時に使える。
//   IMU(MPU6886, I2C): G38(SDA)/G39(SCL) -- 内蔵。RCB4の予約OPCODE(0x90)を
//     ATOM S3側で横取りしてIMU値を返す(下記【IMU予約OPCODEプロトコル】参照)。
//   I2C(外部拡張用): G1/G2 -- Grove(HY2.0-4P)コネクタ。まだ未使用。
...
// M5StickV(K210, MaixPy object_detection_I2C_slave)とのI2Cブリッジ用予約OPCODE。
// G1/G2(外部Grove I2C、無印AtomS3ではSCL=G1/SDA=G2 -- M5Unifiedのボード定義より)
// にI2Cハブ経由で複数台ぶら下げる想定。レジスタマップはM5StickV側ファームウェア
// (~/rcb4eus/Arduino/M5StickV/object_detection_I2C_slave)準拠:
//   reg 0x00: 制御(bit3=apriltag_en等)  reg 0x01: 検出データ長  reg 0x02-: 検出データ本体
// PCから来たコマンド [length, 0x91, i2c_addr, subcmd, (subcmd=1: value / subcmd=3:
// regAddr,value), checksum] を実RCB4には転送せず横取りする。
//   subcmd 0 (READ)      : i2c_addrのreg0x01→reg0x02...を読み出して返す
//   subcmd 1 (WRITE)     : i2c_addrのreg0x00(制御レジスタ)にvalueを書き込む
//   subcmd 2 (SCAN)      : I2Cバスをスキャンして繋がっているアドレス一覧を返す
//   subcmd 3 (WRITE_REG) : i2c_addrの任意レジスタ(regAddr)にvalueを書き込む
//                          (2026.8追加。ADDR_SPEAK_ENABLE(0xFE)等、制御レジスタ
//                          以外への書き込みが必要になったため。subcmd 1は
//                          既存呼び出し元との互換のためレジスタ0x00固定のまま残す)
static const uint8_t M5STICKV_OPCODE = 0x91;
static const int M5STICKV_SDA_PIN = 2;  // G2
static const int M5STICKV_SCL_PIN = 1;  // G1
static const int M5STICKV_MAX_READ = 60;
volatile bool m5stickvRequestPending = false;
volatile uint8_t m5stickvReqAddr = 0;
volatile uint8_t m5stickvReqSubcmd = 0;
volatile uint8_t m5stickvReqValue = 0;
volatile uint8_t m5stickvReqRegAddr = 0;

static const char *MY_NAME = "ATOMS3-ROBOT-I2C";
...
// RCB4-mini 側 UART 設定。ATOM S3底面の拡張用GPIOパッド(G5/G6)。
// RCB4-miniのCOMコネクタはGND側から GND-Rx-Tx の順(HeartToHeart4マニュアルより)。
// G1(RX)/G2(TX)版と同じ相対順序でG5(RX)/G6(TX)に割り当てる。
static const int RCB4_TX_PIN = 6;
static const int RCB4_RX_PIN = 5;
...
void onDataRecv(const esp_now_recv_info_t *info, const uint8_t *data, int len) {
  ...
  if (data[0] == PKT_DATA && len > 2) {
    ...
    if ((int32_t)seq != lastProcessedSeq) {
      ...
      } else if (len - 2 >= 4 && data[3] == M5STICKV_OPCODE) {
        // M5StickV I2Cブリッジ予約OPCODE: 実RCB4には転送せず、loop()側でI2C
        // トランザクションを行って返信する(ESP-NOW受信コールバックからは
        // 重いI2C通信を避けるため、IMU予約OPCODEと同様にフラグ経由で処理)。
        m5stickvReqAddr = data[4];
        m5stickvReqSubcmd = data[5];
        if (m5stickvReqSubcmd == 1 && len - 2 >= 5) {
          m5stickvReqValue = data[6];
        } else if (m5stickvReqSubcmd == 3 && len - 2 >= 6) {
          m5stickvReqRegAddr = data[6];
          m5stickvReqValue = data[7];
        }
        m5stickvRequestPending = true;
      } else {
...
// M5StickV I2Cブリッジ予約OPCODEの応答フレームを組み立て、実RCB4応答と同じ
// 経路(sendChunk)で送る。subcmd=0(READ)は [respLen,0x91,addr,0,i2c_ok,L,data...,cksum]、
// subcmd=1(WRITE)は [respLen,0x91,addr,1,i2c_ok,cksum] (payload無し)。
// i2c_ok: そのアドレスへのI2Cトランザクション自体が成功したか(1=ACK, 0=NACK/未応答
// -- レジスタ長0との区別のため必須。endTransmission()の戻り値0=成功)。
void sendM5StickVReply(uint8_t addr, uint8_t subcmd) {
  uint8_t frame[7 + M5STICKV_MAX_READ];
  uint8_t total;  // フレーム総バイト数(自身のlengthバイト+checksum込み)

  if (subcmd == 2) {
    // I2Cバス上に実際に何が繋がっているかをスキャンする(アドレス0x24/0x25の
    // 決め打ちに頼らず確認するため)。応答: [respLen,0x91,0,2,count,addr...,cksum]
    frame[1] = M5STICKV_OPCODE;
    frame[2] = 0;
    frame[3] = 2;
    uint8_t idx = 5;
    uint8_t count = 0;
    for (uint8_t a = 0x08; a <= 0x77 && idx < sizeof(frame) - 1; a++) {
    ...
    Wire.write((uint8_t)0x00);  // 制御レジスタ
    Wire.write(m5stickvReqValue);
    uint8_t err = Wire.endTransmission();
    frame[1] = M5STICKV_OPCODE;
    frame[2] = addr;
    frame[3] = 1;
    frame[4] = (err == 0) ? 1 : 0;
    total = 6;  // [length,opcode,addr,subcmd,i2c_ok,checksum]
  } else if (subcmd == 3) {
    Wire.beginTransmission(addr);
    Wire.write(m5stickvReqRegAddr);  // 任意レジスタ
    Wire.write(m5stickvReqValue);
    uint8_t err = Wire.endTransmission();
    frame[1] = M5STICKV_OPCODE;
    frame[2] = addr;
    ...
    uint8_t dataLen = 0;
    Wire.beginTransmission(addr);
    Wire.write((uint8_t)0x01);  // データ長レジスタ
    uint8_t err = Wire.endTransmission(false);
    uint8_t i2cOk = 0;
    if (err == 0 && Wire.requestFrom((int)addr, 1) == 1) {
      dataLen = Wire.read();
      i2cOk = 1;
    }
    if (dataLen > M5STICKV_MAX_READ) dataLen = M5STICKV_MAX_READ;

    uint8_t got = 0;
    if (i2cOk && dataLen > 0) {
      Wire.beginTransmission(addr);
      Wire.write((uint8_t)0x02);  // データ本体レジスタ
      if (Wire.endTransmission(false) == 0) {
        got = Wire.requestFrom((int)addr, (int)dataLen);
      }
    }
    ...
    frame[2] = addr;
    frame[3] = 0;
    frame[4] = i2cOk;  // そのアドレスにI2Cデバイスが応答したか
    frame[5] = got;    // 実際に読めたバイト数
    uint8_t idx = 6;
    for (uint8_t i = 0; i < got; i++) frame[idx++] = Wire.read();
    total = idx + 1;  // + checksum
  }

  frame[0] = total;
  uint16_t sum = 0;
  for (uint8_t i = 0; i < total - 1; i++) sum += frame[i];
  frame[total - 1] = sum & 0xFF;

  sendChunk(frame, total);
}
...
void loop() {
  ...
  // M5StickV I2Cブリッジ予約OPCODEリクエストの処理(同上の理由でフラグ経由)。
  if (m5stickvRequestPending) {
    m5stickvRequestPending = false;
    sendM5StickVReply(m5stickvReqAddr, m5stickvReqSubcmd);
  }

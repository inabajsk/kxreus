...
90度回転には2方向(`IMU_YZ_SIGN`=`1`または`-1`)あり、実機の生データを
`atominterface.l`の`orient-coords-to-axis`(実際に`:real-orientation`が
使っている変換そのもの)へ通してシミュレーションした結果、`-1`の方で
通常姿勢がちょうど直立(roll=pitch=yaw=0)になることを確認した(既定値)。
- `IMU_YZ_SIGN`(既定`-1`、実機確認済み): それでも`:timer-on`中に
  ロボットの前後の傾きが実機と逆に見える場合は`1`に戻すこと。

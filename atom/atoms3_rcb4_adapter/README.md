...
USB-Cケーブル接続の都合上、この構成はESP-NOW版
(`../s3_echo_bridge/atoms3_simple_robot/`)とはAtomS3をZ軸まわりで反転した
向きに取り付ける想定。`.ino`冒頭の`MOUNTED_UPSIDE_DOWN`を1箇所書き換える
だけで、液晶の天地(180度回転)とIMUの加速度az・角速度gzの符号反転が
連動して切り替わるようにしてある(液晶だけ、IMUだけ、と片方を直し忘れて
食い違う事故を防ぐため)。取り付け方向を変えた場合はここを確認すること。

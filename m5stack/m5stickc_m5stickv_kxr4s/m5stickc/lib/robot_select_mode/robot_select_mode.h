...
/// Compared against each of the three robots' own compiled-in id list
/// (policy::servoIdsOfRobot()), the fewest differences wins and becomes
/// the starting candidate -- a suggestion, not an answer: still only a
/// click away from being overridden, and still not written anywhere
/// until the same double click every other path here already needs. No
/// RCB-4 attached (or one that never answers) leaves the candidate at
/// whatever this unit already is, same as before this existed.
...
private:
    ...
    /// its final 5-axis one) will never score 0 against any of the three
    /// full robots, and the raw id list is what lets an operator tell
    /// "this is normal for how far the build has got" from "something is
    /// actually wired wrong" -- the diff count alone cannot say which.

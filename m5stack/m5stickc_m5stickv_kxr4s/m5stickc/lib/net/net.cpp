...
bool handleSetupLine(const char* line, Print& out) {
    ...
    if (strcmp(line, "robot?") == 0) {
        // Which of the three robots kxr4s's own NVS (see policy::robotId())
    ...
    if (strncmp(line, "robot ", 6) == 0) {
        // robot <kxrl4t|kxrl4d|kxrl6> -- see policy.h's own setRobotId()
        // comment for why this stores to NVS but does not switch anything
        // running right now: it takes effect on the next boot, the one
        // point where nothing has already loaded the old robot's own
        // weights/servo mapping into a live control loop.

...
// Every robot kxr4s carries shares these -- confirmed identical across
...
/// robot's own obs/act dim -- kxr4s carries three robots' worth of actors
...
/// observations, the same bound the AtomS3 kxr4s tree (which also carries
...
///
/// kxr4s carries every one of kxrl4t/kxrl4d/kxrl6's own trained actors in
...
/// ~/kxreus/m5stack/atoms3_m5stickv_kxr4s), not this board's, so an
...
/// RobotId::COUNT if `name` does not match any of the three -- callers
...
/// three of kxrl4t/kxrl4d/kxrl6 use 0, but which robot's body an AtomS3
/// ends up mounted in decides this now, not which firmware image it
/// happened to be flashed with).
int displayRotation();

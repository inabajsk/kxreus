...
/// kxr4s carries every one of kxrl4t/kxrl4d/kxrl6's own trained actors
...
// enum exactly (KXRL4T, KXRL4D, KXRL6): kRobots[static_cast<size_t>(id)]
// is how every accessor below reaches "the configured robot's own
// slice", with no name lookup on the hot path.
// =======================================================================
...
struct RobotEntry {
    ...
    /// that actor's own OBS_DIM/LAYERS/WEIGHT_FLOATS.
    size_t upload_bytes;
};
...
// The largest of the three robots' own upload_bytes above (kxrl4d's,
// today) -- what g_upload_buf is actually malloc'd to, once, in begin(),
...
// Not defined for this tree's own platformio.ini -- kxr4s's upload buffer

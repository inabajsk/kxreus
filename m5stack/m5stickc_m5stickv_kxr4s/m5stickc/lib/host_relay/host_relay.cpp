...
/// Which of kWords the current line committed to, chosen from its very
/// first byte (see feedSetup()) -- "net" and "robot" never share a first
/// letter, so this is decided once and never ambiguous.
const char* g_setup_word = nullptr;
size_t g_setup_word_len = 0;
...
/// "net?"/"net!"/"net " prefix match -- generalized here to also recognize
/// "robot?"/"robot " (kxr4s's own per-unit robot identity command, see
/// net::handleSetupLine()'s own "robot " branch). "net" and "robot" never
/// share a first letter, so which one a line committed to is decided once,
/// from that very first byte (g_setup_word/g_setup_word_len below), and
/// every byte after is validated against just that one word -- same as the
/// original single-word version, just no longer a hardcoded literal and
/// length. Past g_setup_word_len's own separator, there is deliberately no
/// further validation at all (free-form ssid/password or robot name),
/// exactly as the original had no case at all for a position past 3.
bool feedSetup(Rcb4Link& link, uint8_t byte) {
    static const char* const kWords[] = {"net", "robot"};
    constexpr size_t kWordCount = sizeof(kWords) / sizeof(kWords[0]);

    if (!g_in_setup) {
        const char* word = nullptr;
        for (size_t i = 0; i < kWordCount; i++) {
            if (byte == static_cast<uint8_t>(kWords[i][0])) {
                word = kWords[i];
                break;
            }
        }
        if (word == nullptr || link.midFrame()) return false;
        g_in_setup = true;
        g_setup_len = 0;
        g_setup_word = word;
        g_setup_word_len = strlen(word);
    } else if (g_setup_len < g_setup_word_len) {
        if (byte != static_cast<uint8_t>(g_setup_word[g_setup_len])) {

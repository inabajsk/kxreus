...
/// Which physical robot THIS unit is currently configured as (see
/// policy::robotId()), or a generic placeholder before setRobotId() has
/// ever been called on it. Printed under every mode's title on the LCD --
/// with the same kxr4s firmware image running on any of three different
/// robots' own M5StickC, "which one is this" has to be answerable by
/// looking at the one screen the firmware has, not by tracing a USB
/// cable, and it can no longer be baked in at compile time the way a
/// single-robot tree's own ROBOT_NAME build flag was.
inline const char* robotName() {
    return policy::robotConfigured() ? policy::robotIdName(policy::robotId())
                                      : "kxr4s";
}

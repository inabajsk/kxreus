...
#include <rcb4_link.h>  // Rcb4Link::M5STICKV_MAX_READ
...
struct Telemetry {
    ...
    /// The M5StickV I2C bridge's last poll (see setM5StickVTelemetry(),
    /// called from PolicyMode's own periodic poll -- deliberately not every
    /// control step, since this is a separate I2C bus an operator only
    /// glances at, not a value the policy itself needs). m5stickv_ok is
    /// whether that I2C address answered at all (Wire's own
    /// endTransmission() == 0) -- distinct from m5stickv_len being 0, which
    /// is a real answer saying "nothing detected right now".
    bool m5stickv_ok;
    uint8_t m5stickv_len;
    uint8_t m5stickv_data[Rcb4Link::M5STICKV_MAX_READ];
    /// The control register (reg 0x00: bit2 nn_en, bit3 apriltag_en, bit4
    /// red_en, bit5 green_en, bit6 april3d_en, bit7 cube_face_en -- see
    /// kM5vPage's own comment) and the speak-enable register (reg 0xFE:
    /// bit0 speak_cube_en, bit1 speak_face_en, bit2 speak_apriltag_en),
    /// read back each poll so the settings page shows what this M5StickV
    /// actually has right now, not merely what was last asked for.
    uint8_t m5stickv_ctrl;
    uint8_t m5stickv_speak_ctrl;
};
...
/// Publishes one M5StickV poll (see Telemetry's own m5stickv_ok/
/// m5stickv_len/m5stickv_data/m5stickv_ctrl/m5stickv_speak_ctrl). Separate
/// from setTelemetry() because it runs on its own, much slower cadence
/// (PolicyMode's periodic poll, not every control step -- an I2C
/// transaction here is not something the control loop's own timing budget
/// should ever pay for).
void setM5StickVTelemetry(bool ok, const uint8_t* data, uint8_t len,
                          uint8_t ctrl, uint8_t speak_ctrl);

/// The phone's M5StickV settings page (kM5vPage) asking to flip a
/// register -- handed to PolicyMode's own poll to actually perform (see
/// Rcb4Link::writeM5StickVReg()), the same reasoning as every other
/// radio/hardware request in this file that is set here and consumed
/// elsewhere rather than acted on by the task that received it.
void requestM5StickVWrite(uint8_t reg_addr, uint8_t value);

/// Consumed by PolicyMode's poll once per cycle: true (and both out
/// params filled) if a write was waiting, false (and neither touched) if
/// not. Clears the request either way it returns true, so the same write

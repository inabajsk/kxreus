...
void RobotSelectMode::detectFromHardware() {
    ...
    for (size_t r = 0; r < static_cast<size_t>(policy::RobotId::COUNT); r++) {
        ...
        // slots -- kxrl2g's own head/gripper joints (see policy::
        // servoDirection()'s own comment on the differential-linkage
        // grippers) carry ids >= SERVO_SLOTS or repeat one id twice for
        // two joints; both fall out for free here, the first by the
        // bounds check and the second because a bitmap has no concept
        // of "twice".
...
void RobotSelectMode::draw() {
    ...
    if (just_applied_) {
        ...
        if (detected_valid_) {
            ...
            // to 35 two-digit ids this can run to several lines.
            M5.Display.setTextColor(TFT_WHITE, TFT_BLACK);

...
/// kxr4s carries every one of kxrl4t/kxrl4d/kxrl6/kxrl2g's own trained
...
// enum exactly (KXRL4T, KXRL4D, KXRL6, KXRL2G): kRobots[static_cast<
...
// The largest of the four robots' own upload_bytes above (kxrl2g's, today)
// -- what g_upload_buf is actually malloc'd to, once, in begin(), same as
// every other robot's own build did for its one shape. Sized to the worst
// case rather than to whichever robot ends up configured on THIS unit: the
// same compiled firmware image is flashed onto all of them, before
// setRobotId() has said which, and the buffer cannot grow later.
...
// Not defined for this tree's own platformio.ini -- kxr4s's ~123 KiB

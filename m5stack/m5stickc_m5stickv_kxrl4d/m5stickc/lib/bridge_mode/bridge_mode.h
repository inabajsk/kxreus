...
private:
    ...
    void releaseSetup();
    /// 16 px apart, back to back with the title row above and the button
    /// legend below it -- see drawLamp()'s own comment on why its row
    /// shrank to match.
    static constexpr int BOARD_LAMP_Y = 16;
    static constexpr int IMU_LAMP_Y = 32;
    ...
    static constexpr int I2C_LAMP_Y = 48;

...
// buffer (kUploadTotalBytes below), sized to the WALK actor's own shape --
// an upload always replaces "walk", never "getup".
//
...
// this file for the real-hardware heap-fragmentation failure this
// sidesteps: plenty of free heap overall, but no single run big enough
// left in it once the Wi-Fi/HTTP stack has made its own allocations.
...
bool select(size_t index) {
    ...
    if (a.quantized) {
        ...
        // upload buffer itself, malloc'd whole in begin() -- so there is
        // no flash-vs-RAM choice for the block below to make, and
        // nothing to do with g_weights/g_biases: run() reads

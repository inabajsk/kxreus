...
def main():
    ...
    # The very first thing tried in a fresh raw-REPL session, right after
    # this script interrupted the board as early as it possibly could (see
    # the aggressive-Ctrl-C comment above) -- which occasionally lands
    # before the K210's own SD card driver has finished (re)mounting after
    # the reboot this script itself just caused. That shows up as ENODEV
    # from open(), not from anything about the file or the write itself
    # (confirmed on real hardware, 2026.9: two clips uploaded fine back to
    # back, the third failed here on the very first raw-REPL command of its
    # own fresh session). A short retry covers exactly that race without
    # masking a real, persistent SD problem (which would still fail after
    # several seconds of retrying).
    for attempt in range(10):
        try:
            exec_raw(ser, b"f = open('%s', 'wb')\r\n" % remote_name.encode())
            break
        except RuntimeError as e:
            if "ENODEV" not in str(e) or attempt == 9:
                raise
            print("  [retry] SD card not ready yet (ENODEV), retrying...")
            time.sleep(0.3)

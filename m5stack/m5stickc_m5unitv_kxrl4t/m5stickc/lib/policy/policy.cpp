...
// upload buffer (kUploadTotalBytes below).
//
...
// than malloc'd fresh on every beginUpload(). Malloc'ing fresh on every
// call used to fail on real hardware (a sibling build of this same
// robot) with plenty of free heap overall but no single contiguous run
// that size left after the HTTPS server, ESP-NOW, and Wi-Fi stack had
...
bool select(size_t index) {
    ...
    if (a.quantized) {
        ...
        // upload buffer itself, filled whole in finishUpload() -- so

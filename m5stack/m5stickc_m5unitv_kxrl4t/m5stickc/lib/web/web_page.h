...
// compare against. Shared with the "download" section below (which
// fetches the same shape of file from GitHub instead of a local picker)
// through this one call, so the robot's own network path -- whichever
// source the bytes came from -- is always this exact multipart POST.
// An earlier version of a sibling build had the ROBOT itself fetch a
// download from GitHub directly, which broke that board's own HTTPS
// server on real hardware (a second, unrelated outbound TLS connection
// while its own TLS server was up did not coexist reliably); routing
// every source through this SAME upload call keeps this robot's own
// server completely out of it either way.
function uploadPolicyBlob(blob, doneMessage) {
  ...
  xhr.onload = async () => {
    if (xhr.status === 200) {
      ...
      // No phone-side actor picker on this build (unlike kxrl4t) -- just
      // keep ACTORS current for the status line's own display (see
      // tick()'s "· ${ACTORS[...]}" use of it) so it names "uploaded"
      // correctly once whatever already selects actors on this robot
      // reaches it.
      try {
        const r = await fetch('/info', {cache: 'no-store'});
...
document.getElementById('policyGo').addEventListener('click', () => {
  ...
  uploadPolicyBlob(input.files[0], 'policy: uploaded');
});
...
// raw.githubusercontent.com, on whatever network the phone itself is
// on, regardless of which one the robot joined), then handed to
// uploadPolicyBlob() above -- the same call the file picker uses -- so
// the robot's own network path never changes with the source.
const DOWNLOAD_BASE =
    'https://raw.githubusercontent.com/inabajsk/kxreus/master/' +
    'm5stack/m5stickc_m5unitv_kxrl4t/downloadable_policies/';
let DOWNLOADS = [];
...
// Decodes the M5StickV I2C bridge's raw detection-record bytes into a short
// human-readable summary, mirroring object_detection_I2C_slave.py's own
// TYPE_NN/TYPE_APRILTAG/TYPE_RED/TYPE_GREEN byte layout (see that script's
// own detection_data.append() calls) -- the same decode
// atom_common/lib/status_mode.cpp's drawAprilTag() already does for the
// device's own LCD, done here again because a phone looking at this page has
// no LCD of its own to read. Only the four record types this firmware's own
// control register can normally turn on are decoded (see M5STICKV_OPCODE's
// own comment); anything else (April3D, cube faces -- rare, off by default)
// falls through to a byte count rather than guessing at its shape.
function decodeM5vRecords(bytes) {
  const i16 = (lo, hi) => { const v = (hi << 8) | lo; return v >= 0x8000 ? v - 0x10000 : v; };
  const sizes = {0: 11, 1: 20, 2: 9, 3: 9};
  const out = [];
  let i = 0;
  while (i < bytes.length) {
    const type = bytes[i];
    const size = sizes[type];
    if (size === undefined || i + size > bytes.length) {
      out.push(`${bytes.length - i} unrecognised byte(s)`);
      break;
    }
    const r = bytes.slice(i, i + size);
    if (type === 0) {
      out.push(`NN class ${r[10]} (${Math.round(r[9] / 255 * 100)}%)`);
    } else if (type === 1) {
      const id = i16(r[17], r[18]);
      out.push(`AprilTag #${id} (${Math.round(r[19] / 255 * 100)}%)`);
    } else if (type === 2) {
      out.push('red blob');
    } else {
      out.push('green blob');
...
async function tick() {
  ...
  try {
    ...
    // polls PolicyMode actually does. Decoded into record names (see
    // decodeM5vRecords()) rather than shown as raw hex: "AprilTag #3 (91%)"
    // says something at a glance a byte dump does not.
    if (Array.isArray(t.m5v_data)) {
      const recs = decodeM5vRecords(t.m5v_data);
      const summary = recs.length ? recs.join(', ')
                     : t.m5v_data.length ? `${t.m5v_data.length} bytes, none decoded`
                     : 'nothing detected';
...
/// The M5StickV settings page, at /m5v -- reachable from kWebPage's own
/// "M5StickV" link (see its .m5vsec) and back again via this page's own
/// "Control" link, both plain <a> navigations (no SPA routing here: this
/// is two small pages the device serves at two paths, not one page with
/// client-side state to keep in sync between them).
///
/// Register bits, from object_detection_I2C_slave.py -- unchanged by this
/// page, only exposed:
///   reg 0x00 (control): bit0 save-one-image (one-shot, not offered here
///     -- flipping it back off itself is the M5StickV's own job, and a
///     checkbox that un-checks itself under the phone's feet is more
///     confusing than a plain toggle is worth), bit1 white LED, bit2
///     nn_en (face/general object detection), bit3 apriltag_en, bit4
///     red_en, bit5 green_en (colour blob detection), bit6 april3d_en,
///     bit7 cube_face_en.
///   reg 0xFE (speak-enable, this project's own TTS cues): bit0
///     speak_cube_en, bit1 speak_face_en, bit2 speak_apriltag_en.
/// Both default to 0x00/0x07 respectively at boot (0xFE, unlike 0x00, is
/// *not* all zero out of the box) -- see that file's own __init__. An
/// apriltag_en of 0 does not mean apriltag detection never runs at all:
/// speak_apriltag_en defaulting to 1 already triggers it (for the spoken
/// announcement), which is also why a blue box can already appear before
/// this page ever writes anything -- but bit3 is what a caller other than
/// the speak feature can rely on being on.
const char kM5vPage[] PROGMEM = R"HTML(<!doctype html>
<html lang="en"><head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>M5StickV</title>
<style>
  body { margin:0; font:15px/1.4 -apple-system,system-ui,sans-serif;
         background:#111; color:#eee; padding:14px 14px 28px;
         box-sizing:border-box; }
  h1 { font-size:17px; margin:0 0 2px; }
  a.back { color:#7ab8ff; font-size:14px; text-decoration:none; }
  #status { font-size:12px; color:#9aa; margin:10px 0 16px;
            font-variant-numeric:tabular-nums; word-break:break-all; }
  h2 { font-size:13px; color:#9aa; margin:18px 0 6px; font-weight:600; }
  label.row { display:flex; align-items:center; gap:10px; padding:9px 2px;
              border-bottom:1px solid #222; font-size:15px; }
  label.row input { width:auto; flex:0 0 auto; }
</style></head><body>
<a class="back" href="/">&larr; Control</a>
<h1>M5StickV</h1>
<div id="status">loading…</div>
<h2>Detection (register 0x00)</h2>
<div id="ctrl"></div>
...
const CTRL_BITS = [
  ...
  [4, 'Red blob detection'],
  [5, 'Green blob detection'],
  [6, 'AprilTag 3D pose'],
  [7, 'Cube face colours'],
  [1, 'White LED'],
];
...
function buildRows(container, bits, reg) {
...
buildRows(document.getElementById('ctrl'), CTRL_BITS, 0);
buildRows(document.getElementById('speak'), SPEAK_BITS, 0xfe);

// Registers as this page last knew them -- kept so a single checkbox
// toggle can be turned into "the whole byte with just that bit flipped"
// without a round trip first, and so the next poll's own read does not
// fight a write still in flight (see applying, below).
let ctrlByte = 0, speakByte = 0;
let applying = false;

function paint() {
  for (const box of document.querySelectorAll('input[type=checkbox]')) {
    const reg = Number(box.dataset.reg), bit = Number(box.dataset.bit);
    const byte = reg === 0xfe ? speakByte : ctrlByte;
    box.checked = ((byte >> bit) & 1) === 1;
  }
}

document.getElementById('ctrl').addEventListener('change', onToggle);
document.getElementById('speak').addEventListener('change', onToggle);
async function onToggle(e) {
  const box = e.target;
  if (box.tagName !== 'INPUT') return;
  const reg = Number(box.dataset.reg), bit = Number(box.dataset.bit);
  let byte = reg === 0xfe ? speakByte : ctrlByte;
  byte = box.checked ? (byte | (1 << bit)) : (byte & ~(1 << bit));
  if (reg === 0xfe) speakByte = byte; else ctrlByte = byte;
  applying = true;
  try {
    await fetch('/m5vset', {
      method: 'POST',
      headers: {'Content-Type': 'application/x-www-form-urlencoded'},
      body: `reg=${reg}&value=${byte}`,
    });
  } catch (e) {
    // The next poll below will show whatever actually took (or didn't) --
    // this page does not retry on its own.
  } finally {
    applying = false;
  }
}

// See kWebPage's own copy of this function for the full comment -- this
// page is a separate <script> (a plain page navigation, not client-side
// routing, per this file's own top comment), so nothing is shared between
// the two and the decoder is simply duplicated rather than factored out.
function decodeM5vRecords(bytes) {
  const i16 = (lo, hi) => { const v = (hi << 8) | lo; return v >= 0x8000 ? v - 0x10000 : v; };
  const sizes = {0: 11, 1: 20, 2: 9, 3: 9};
  const out = [];
...
async function poll() {
  try {
    // No "f" argument: handleCommandRequest() only ACTS on a command frame
    // when one is given, but always reports telemetry regardless -- this
    // page reads it without ever sending one, so it can never be the thing
    // that changes what the robot is doing (see kWebPage's own frame(),
    // the only place a real command belongs).
    ...
    // A write this page just sent is applied on the SAME poll cycle that
    // reads it back (see PolicyMode's own comment on why), but the two
    // still race across two separate HTTP round trips -- skip repainting
    // from a read that could be showing the byte from just before this
    // page's own most recent write.
    if (!applying) {
      ctrlByte = t.m5v_ctrl;
      speakByte = t.m5v_speak;
      paint();
    }
    const recs = decodeM5vRecords(t.m5v_data ?? []);
    const summary = recs.length ? recs.join(', ')

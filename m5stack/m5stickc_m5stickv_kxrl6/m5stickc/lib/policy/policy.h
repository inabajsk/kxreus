...
//
// Bound to kxrl6walk rather than an arbitrary actor because every actor
// compiled into THIS build (kxrl6's only actor: walk) shares one obs/action
// layout and control rate -- they are the same robot's two skills, not two
// different robots the way crawl/omni/etc were.
#define POLICY_OBS_DIM POLICY_KXRL6WALK_OBS_DIM
#define POLICY_ACT_DIM POLICY_KXRL6WALK_ACT_DIM
#define POLICY_SERVO_FRAME_COUNT POLICY_KXRL6WALK_SERVO_FRAME_COUNT
#define POLICY_HOME_FRAME_COUNT POLICY_KXRL6WALK_HOME_FRAME_COUNT
#define POLICY_SERVO_STRETCH POLICY_KXRL6WALK_SERVO_STRETCH
#define POLICY_CONTROL_HZ POLICY_KXRL6WALK_CONTROL_HZ
#define POLICY_JOINT_VEL_WINDOW_S POLICY_KXRL6WALK_JOINT_VEL_WINDOW_S
#define POLICY_OBS_BASE_ANG_VEL_START POLICY_KXRL6WALK_OBS_BASE_ANG_VEL_START
#define POLICY_OBS_PROJECTED_GRAVITY_START POLICY_KXRL6WALK_OBS_PROJECTED_GRAVITY_START
#define POLICY_OBS_COMMAND_START POLICY_KXRL6WALK_OBS_COMMAND_START
#define POLICY_OBS_PHASE_START POLICY_KXRL6WALK_OBS_PHASE_START
#define POLICY_OBS_JOINT_POS_START POLICY_KXRL6WALK_OBS_JOINT_POS_START
#define POLICY_OBS_JOINT_VEL_START POLICY_KXRL6WALK_OBS_JOINT_VEL_START
#define POLICY_OBS_ACTIONS_START POLICY_KXRL6WALK_OBS_ACTIONS_START
...
/// float32: this is noise next to the weights+biases below, and they
/// feed the very first subtraction/multiply every observation goes
/// through, where a bad scale would cost more than it saves.
///

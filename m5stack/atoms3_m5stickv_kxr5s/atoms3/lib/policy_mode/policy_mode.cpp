...
// Independent of DRAW_INTERVAL_MS on purpose -- see draw()'s own
// comment; this just happens to also live on that task.
constexpr uint32_t M5STICKV_POLL_INTERVAL_MS = 1000;
...
/// A joint whose real RCB-4 id has not been verified against hardware is
/// given this sentinel instead of a guessed number -- sending a plausible but
/// wrong id would drive whatever servo actually sits at that slot. kxrl2g's
/// head (2 axes, not standardised across body variants -- see kxrl4d/kxrl4t's
/// own copy of this comment) AND both gripper joints per arm are unwired for
/// this reason: the real board drives larm-gripper-r/larm-gripper2-r (and
/// the right arm's pair) from a SINGLE physical servo with the logical angle
/// halved (kxreus/rcb4robotconfig.l's "1/2" weight, a differential linkage
/// this firmware's one-id-per-joint model has no scale factor for). Legs and
/// arm shoulder/elbow are confirmed, 1:1, real ids.
...
void buildServoOrder() {
    ...
    // Bound by the CONFIGURED robot's own actDim(), not the POLICY_ACT_DIM
    // capacity macro -- every array here is sized to that capacity, but a
    // robot with fewer joints than the widest one (kxrl2g's 22) must stop
    // short of it: policy::servoIds()/homeRad()/etc for the configured
    // robot's own compiled-in actor are exactly actDim() long in flash,
    // and reading past that end is an out-of-bounds read of whatever
    // .rodata happens to sit next.
    const size_t act_dim = policy::actDim();
    for (size_t i = 0; i < act_dim; i++) g_sorted_to_joint[i] = i;
    // Insertion sort: at most 22 elements, once, at startup.
...
void PolicyMode::pinUnwiredJoints() {
    for (size_t i = g_servo_count; i < policy::actDim(); i++) {
...
void PolicyMode::enter() {
    ...
    //
    // Skipped when the configured robot carries no actor at all (kxra6g,
    // see policy::RobotId's own comment): policy::homeRad() has nothing
    // real to return then (actorAt() falls back to the unfinished
    // "uploaded" actor slot, home_rad still null) and last_target_ is
    // never read for such a robot anyway -- loop()'s own policy::count()
    // == 0 branch below never reaches RUN/HOLD/RISE/SIT, the only
    // consumers of last_target_.
    if (policy::count() > 0) {
        memcpy(last_target_, policy::homeRad(), sizeof(last_target_));
    } else {
        memset(last_target_, 0, sizeof(last_target_));
    }
    overruns_ = 0;
    ...
    if (draw_task_ == nullptr) {
        ...
        //
        // 4096 was enough before this task also grew a Telemetry snapshot
        // (net::telemetry()'s return, now carrying the M5StickV detection
        // array too), a second same-sized buffer for the M5StickV poll's
        // own read, and the QR code screen's own rendering (M5.Display.
        // qrcode(), which does its own encoding work on this same call
        // stack) -- all in the one function (draw()) this task runs.
        // Bumped rather than trusting the old number was still enough: a
        // stack overflow here does not throw, it corrupts something next
        // to it, which on real hardware showed up as the whole device
        // going unreachable rather than a clean crash pointing at this.
        xTaskCreatePinnedToCore(drawTask, "kxr-lcd", 6144, this, 1,
                                &draw_task_, 0);
    }
}
...
void PolicyMode::draw() {
    // The M5StickV I2C bridge (see Rcb4Link::M5STICKV_OPCODE's own
    // comment), polled here rather than from the control loop: this is a
    // separate bus with nothing to do with the servos, and an operator
    // glancing at the phone's display is not something the control loop's
    // own timing budget should ever pay for -- see last_gravity_root_'s own
    // comment for the same reasoning applied to a second IMU read once
    // already. Runs on this task's own core (0), well away from core 1's
    // control loop, at its own slower cadence (see next_m5stickv_poll_ms_).
    if (static_cast<int32_t>(millis() - next_m5stickv_poll_ms_) >= 0) {
        next_m5stickv_poll_ms_ = millis() + M5STICKV_POLL_INTERVAL_MS;
        // A phone-requested toggle (see net::requestM5StickVWrite(), from
        // kM5vPage) is applied here, on this same task, right before the
        // read below -- so the next reported ctrl/speak values already
        // reflect it, rather than waiting a further cycle to catch up.
        uint8_t write_reg = 0, write_value = 0;
        if (net::takeM5StickVWrite(&write_reg, &write_value)) {
            link_.writeM5StickVReg(Rcb4Link::M5STICKV_DEFAULT_ADDR, write_reg, write_value);
        }
        uint8_t data[Rcb4Link::M5STICKV_MAX_READ];
        uint8_t len = 0;
        const bool ok = link_.readM5StickV(Rcb4Link::M5STICKV_DEFAULT_ADDR, data, &len);
        // Skipped once the M5StickV has already failed to answer this
        // cycle: two more transactions to the same silent address is bus
        // time (and, on real hardware, however this platform's I2C driver
        // behaves when a device is not there to NACK) worth not spending
        // on an address that just proved unresponsive.
        uint8_t ctrl = 0, speak_ctrl = 0;
        if (ok) {
            link_.readM5StickVReg(Rcb4Link::M5STICKV_DEFAULT_ADDR, 0x00, &ctrl);
            link_.readM5StickVReg(Rcb4Link::M5STICKV_DEFAULT_ADDR, 0xFE, &speak_ctrl);
        }
        net::setM5StickVTelemetry(ok, data, len, ctrl, speak_ctrl);
    }
    ...
    // No robot identity set on this unit (see policy::robotConfigured()'s
    // own comment and loop()'s own guard, which keeps this stuck at IDLE
    // regardless) -- overrides whatever the state switch above picked, so
    // an operator sees WHY nothing responds rather than a plain IDLE that
    // looks identical to a configured robot simply not running yet.
    if (!policy::robotConfigured()) {
        label = "NOROBOT";
        colour = TFT_RED;
    }
...
void PolicyMode::sendTelemetry() {
    ...
    float peak = 0.0f;
    for (size_t i = 0; i < policy::actDim(); i++) {
...
void PolicyMode::buildObs(float* obs) const {
    memset(obs, 0, sizeof(float) * policy::obsDim());
    ...
    // OBS_JOINT_VEL_START/OBS_ACTIONS_START are not compiled-in constants
    // any more (see policy.h's own comment): joint pos, then joint vel,
    // then last action, each actDim() wide, packed back to back from
    // OBS_JOINT_POS_START -- computed here rather than read from a
    // constant that would be wrong for every robot but one.
    const size_t act_dim = policy::actDim();
    const size_t obs_joint_vel_start = POLICY_OBS_JOINT_POS_START + act_dim;
    const size_t obs_actions_start = POLICY_OBS_JOINT_POS_START + 2 * act_dim;
    for (size_t i = 0; i < act_dim; i++) {
        ...
        obs[obs_joint_vel_start + i] = joint_vel_[i];
        obs[obs_actions_start + i] = last_action_[i];
    }
}
...
bool PolicyMode::step() {
    ...
    // homeRad() is exactly actDim() long in flash (the configured robot's
    // own compiled-in array) -- looping to the POLICY_ACT_DIM capacity
    // macro instead would read past its end for every robot but the
    // widest one.
    const size_t act_dim = policy::actDim();
...
void PolicyMode::beginRamp(const float* target, float seconds) {
    ...
    // target is policy::homeRadOf()'s own return -- exactly actDim() long
    // in flash, same reasoning as writeJointTargets()'s own comment above.
    float span = 0.0f;
    for (size_t i = 0; i < policy::actDim(); i++) {
        ramp_to_[i] = target[i];
...
void PolicyMode::loop() {
    ...
    // No safe default to guess (see policy::robotConfigured()'s own
    // comment): every one of the four robots' own servo_ids/
    // servo_direction is meaningless applied to a different one's
    // wiring, so an AtomS3 that has never had its robot identity set
    // (see net.cpp's own "robot <name>" serial command) must not drive a
    // single servo -- stuck at IDLE with servos free, no matter what is
    // asked, the same way net::Status::UNCONFIGURED already refuses to
    // guess a Wi-Fi network rather than picking one.
    if (!policy::robotConfigured()) {
    ...
    // The configured robot carries no trained actor at all (kxra6g -- see
    // policy::RobotId's own comment): RUN/HOLD/RISE/SIT all eventually
    // read policy::homeRad() to know where to ramp the hand to, which for
    // this robot is null (actorAt() falls back to the empty "uploaded"
    // actor slot, never installed), so none of those four can run here --
    // this robot's own movement is the RCB-4's own onboard motion table,
    // not a policy on this chip. Request::MOTION is the one exception:
    // beginMotion() only calls Rcb4Link::callMotion(), never touching an
    // actor, so it is let straight through -- same edge-detection
    // (request_changed, or a different slot asked for while already in
    // MOTION) as the ordinary Request::MOTION case in the switch below,
    // since this branch replaces that switch entirely for this robot
    // rather than running alongside it.
    if (policy::count() == 0) {
        if (request_ == Request::MOTION) {
            if (request_ != last_seen_request_ ||
                motion_number_ != last_motion_number_) {
                last_seen_request_ = request_;

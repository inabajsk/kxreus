...
public:
    ...
    /// The union of every actor role any of kxr5s's five robots' own
    ...
    /// for just one any more) never names GETUP. kxra6g carries neither --
    /// see policy::RobotId's own comment -- and never reaches this at all
    /// (loop()'s own policy::count() == 0 branch returns before the

...
/// from robotName() so several robots' AP names are never the same one) has
...
// kxr5s carries all five robots' own actors in one image (kxra6g's own
// slice is empty -- see lib/policy/policy.h's own RobotId comment), so
// unlike the single-robot tree this page was derived from, WHICH robot's
// own downloadable policies this unit should offer is a runtime fact --
// read from /info
...
// robot's own EXISTING published directory. Paths are relative to this
// repo's own m5stack/ -- kxrl4t/kxrl4d/kxrl6's own downloadable_policies
// live inside the M5StickC kxr5s tree (moved there, not duplicated, since
// the same three robots' data is identical regardless of which board
// fetches it) and kxrl2g's inside this same AtomS3 tree -- both moved out
...
const DOWNLOAD_DIRS = {
  ...
  kxrl2g: 'atoms3_m5stickv_kxr5s/downloadable_policies/kxrl2g',
};
...
async function loadDownloadList() {
  ...
  try {
    const infoR = await fetch('/info', {cache: 'no-store'});
...
// available, without a single tap. Never SELECTS it
// (see uploadPolicyBlob()'s own comment thread on why loading is
// deliberate but selecting stays a separate, human act) -- this only
// makes sure it is not a re-upload away from being run.

...
/// Advertised over mDNS as `<robotName()>.local`, so a PC needs no address --
/// and, with several robots on one network, no address that would collide
/// with another one's either. Not a compile-time constant any more: which
/// robot this unit is comes from NVS (see policy::robotId()), read fresh at
/// each call site rather than cached once, since robotName() itself is
/// cheap (a Preferences read memoized in policy.cpp, not a fresh NVS
...
/// kM5vPage's own toggle, handed off to PolicyMode's poll -- see
/// requestM5StickVWrite()/takeM5StickVWrite()'s own comments. Guarded by
/// g_shared like every other cross-core pending request in this file,
/// unlike g_servo_ids above: this one really can be read mid-write (the
/// phone can ask again before the last ask was even applied).
...
///
/// Built from robotName() rather than a literal, like everything else this
...
/// them apart. An unconfigured unit's own "kxr5s-wifi" is what an operator
/// setting one up for the first time sees before a "robot <name>" command
/// has ever run on it.
String provisionSsid() { return String(robotName()) + "-wifi"; }
DNSServer dnsServer;
...
/// WebServer builds Strings and parses headers; 6 KiB was what that wanted
/// without being generous with a heap the Wi-Fi stack has already taken
/// 52 KiB of. Bumped to 8 KiB once handleCommandRequest()'s own locals
/// (a full Telemetry snapshot, now carrying the M5StickV detection array
/// too, plus the JSON body it is formatted into) grew enough on top of
/// that 6 KiB to be worth not trusting right at the edge -- a stack
/// overflow here reads as this whole device going unreachable, not as a
/// clean error, which is a expensive way to find out it was too tight.
constexpr uint32_t SERVER_STACK = 8192;
...
// Longer than handleScanRequest()'s own worst case (it spins in place for
// up to 8 s waiting for poll()'s scan to finish -- see its own comment)
// with real margin, not just barely: this must never fire during a
// perfectly ordinary slow request.
constexpr uint32_t SERVER_WATCHDOG_TIMEOUT_S = 20;

void serverTask(void*) {
    // A real, permanent hang anywhere in server.handleClient() -- whatever
    // request handler caused it -- used to mean the phone's control page
    // (and everything else this task serves) stayed unreachable until the
    // robot was power-cycled by hand (confirmed on real hardware, 2026.9:
    // no amount of waiting brought it back on its own). Registering this
    // task with the TWDT turns that into an automatic reboot within
    // SERVER_WATCHDOG_TIMEOUT_S instead -- a robot answering again on its
    // own in twenty seconds beats one that needs a hand at the power
    // switch, even without knowing yet which request handler was the one
    // that got stuck.
    //
    // esp_task_wdt_init() returns an error if some other task already
    // initialised the TWDT first (harmless -- same timeout either way,
    // nothing here depends on being the one that actually did it).
    esp_task_wdt_init(SERVER_WATCHDOG_TIMEOUT_S, true);
...
/// regardless of whether PolicyMode's poll gets to it in the next second
/// or the next ten (an M5StickV that is not actually there yet).
void handleM5vSetRequest() {
    if (!server.hasArg("reg") || !server.hasArg("value")) {
        server.send(400, "text/plain", "reg and value required");
        return;
    }
    const long reg = server.arg("reg").toInt();
    const long value = server.arg("value").toInt();
    if (reg < 0 || reg > 255 || value < 0 || value > 255) {
        server.send(400, "text/plain", "reg and value must be 0-255");
        return;
    }
    requestM5StickVWrite(static_cast<uint8_t>(reg), static_cast<uint8_t>(value));
...
void ensureServer() {
    ...
    //
    // STANDALONE_AP, not just g_provisioning: this used to only cover the
    // transient setup form, which meant that in STANDALONE_AP (this robot's
    // own permanent access point, kWebPage's normal home) the same probe got
    // a plain 404 instead -- the DNS half of this same trick already
    // answered both (see poll()'s own dnsServer.processNextRequest() call),
    // so a phone would get redirected to this device at the DNS level but
    // then met a 404 instead of kWebPage at the HTTP level, which on some
    // phones is enough to make the OS decide there is no captive portal
    // here at all and never show the auto-popup browser.
    server.onNotFound([]() {
...
String buildInfoJson() {
    ...
    String body = "{\"robot\":\"" + String(robotName()) + "\",";
    body += range;
    body += "\"actors\":[";
...
void setM5StickVTelemetry(bool ok, const uint8_t* data, uint8_t len,
                          uint8_t ctrl, uint8_t speak_ctrl) {
    portENTER_CRITICAL(&g_shared);
    g_telemetry.t.m5stickv_ok = ok;
    g_telemetry.t.m5stickv_len = len;
    memcpy(g_telemetry.t.m5stickv_data, data, len);
    g_telemetry.t.m5stickv_ctrl = ctrl;
    g_telemetry.t.m5stickv_speak_ctrl = speak_ctrl;
    portEXIT_CRITICAL(&g_shared);
}

void requestM5StickVWrite(uint8_t reg_addr, uint8_t value) {
    portENTER_CRITICAL(&g_shared);
    g_m5v_write_reg = reg_addr;
    g_m5v_write_value = value;
    g_m5v_write_pending = true;
    portEXIT_CRITICAL(&g_shared);
}
...
bool takeM5StickVWrite(uint8_t* reg_addr, uint8_t* value) {
    bool pending;
    portENTER_CRITICAL(&g_shared);
    pending = g_m5v_write_pending;
    if (pending) {
        *reg_addr = g_m5v_write_reg;
        *value = g_m5v_write_value;
        g_m5v_write_pending = false;
    }
    portEXIT_CRITICAL(&g_shared);
    return pending;
}
...
bool handleSetupLine(const char* line, Print& out) {
    ...
    if (strcmp(line, "robot?") == 0) {
        // Which of the five robots kxr5s's own NVS (see policy::robotId())
        // currently says this unit is, or that nothing has said yet --
        // the same "read back what was set" companion "net?" already is
        // for Wi-Fi credentials.
        out.printf("robot=%s configured=%d\n",
                   policy::robotConfigured()
                           ? policy::robotIdName(policy::robotId())
                           : "(unconfigured)",
                   policy::robotConfigured() ? 1 : 0);
        out.println("OK");
        return true;
    }
    if (strncmp(line, "robot ", 6) == 0) {
        // robot <kxrl4t|kxrl4d|kxrl6|kxrl2g> -- see policy.h's own
        // setRobotId() comment for why this stores to NVS but does not
        // switch anything running right now: it takes effect on the next
        // boot, the one point where nothing has already loaded the old
        // robot's own weights/servo mapping into a live control loop.
        const policy::RobotId id = policy::robotIdFromName(line + 6);
        if (id == policy::RobotId::COUNT) {
            out.printf("ERR unknown robot \"%s\" -- expected one of: ",
                       line + 6);
            for (size_t i = 0; i < static_cast<size_t>(policy::RobotId::COUNT);
                 i++) {
                if (i > 0) out.print(", ");
                out.print(policy::robotIdName(static_cast<policy::RobotId>(i)));
            }
            out.println();
            return true;
        }
        if (!policy::setRobotId(id)) {
            out.println("ERR could not store to NVS");
            return true;
        }
        out.printf("OK %s stored -- reboot to apply\n",
                   policy::robotIdName(id));
        return true;
    }

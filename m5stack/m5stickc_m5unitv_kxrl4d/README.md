# kxrl4d — M5StickC + M5Stack UnitV RCB-4 bridge
...
- `m5stickc/` — the RCB-4's COM adapter (PlatformIO/Arduino, ESP32). Relays
  ...
  opcode (0x91) through to the UnitV. Runs kxrl4d's trained walk and getup
  policies in POLICY mode. See `m5stickc/platformio.ini` for the full
  board writeup.
- `m5unitv/` — the camera (K210/MaixPy, `is_m5unitv = True`). Runs
  ...
  (`~/kxreus/m5stack/m5stickc_m5stickv_kxrl4d`), just a different camera

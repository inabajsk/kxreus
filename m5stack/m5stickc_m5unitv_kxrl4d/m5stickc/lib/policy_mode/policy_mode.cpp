...
/// wrong id would drive whatever servo actually sits at that slot. kxrl4d's
/// head (neck + 2 grippers) is unwired for exactly this reason (see
/// tools/export_policy.py's calibration.yaml and lib/policy/policy_spec_
/// kxrl4d{walk,getup}.h): the standard KXR-L4D leg/arm wiring is confirmed
/// against kxreus/rcb4robotconfig.l, the head wiring is not.
...
// RISE: run getup from wherever the hand currently is (getup's own training
// starts from lying flat, but the actor is asked about its OWN observation --
// a standing start converges to the shared home in place) for long enough to
// settle, then hand off. GETUP and WALK share one home pose, so beginActor()
// (which moves nothing) is enough; no RAMP step is needed to bridge them.
const PolicyMode::Step PolicyMode::kRiseSequence[1] = {
        {Step::Kind::RUN, PolicyMode::GETUP, 6.0f},
};

// SIT: kxrl4d has no trained lie-down skill (unlike the wheeled hand's sit),
// so this request just ramps back to the shared home stance and holds there
// under WALK -- the same place a plain `hold` (button click) puts the hand.
...
void PolicyMode::loop() {
    ...
    if (pending_transition_ != Request::FREE &&
        ...
        if (beginSequence(rise ? kRiseSequence : kSitSequence,
                          ...
                          // Both sequences land on WALK: rise because that is
                          // the point of standing up, sit because there is
                          // nothing else trained to land on.
                          WALK)) {
            pending_transition_ = Request::FREE;
        }
    }

...
// The M5StickV bridge's own I2C bus: I2C_NUM_1, entirely separate from
// M5.In_I2C/M5.Ex_I2C (which this board's M5Unified pin table collapses onto
// the SAME I2C_NUM_0 peripheral and GPIOs as the internal IMU -- see
// rcb4_link.h's own M5STICKV_SDA_PIN comment for why that bus is left to the
// IMU alone). A plain Wire second-instance, not M5.Ex_I2C: Port E is not one
// M5Unified itself brings up.
TwoWire M5StickVWire(1);

bool i2cProbe(uint8_t addr) {
    M5StickVWire.beginTransmission(addr);
    return M5StickVWire.endTransmission() == 0;
}

bool i2cReadBytes(uint8_t addr, uint8_t reg, uint8_t* out, size_t len) {
    M5StickVWire.beginTransmission(addr);
    M5StickVWire.write(reg);
    if (M5StickVWire.endTransmission(false) != 0) return false;
    if (M5StickVWire.requestFrom(static_cast<int>(addr),
                                 static_cast<int>(len)) !=
        static_cast<int>(len)) {
        return false;
    }
    for (size_t i = 0; i < len; i++) out[i] = M5StickVWire.read();
    return true;
}
...
void Rcb4Link::beginM5StickV() {
    M5StickVWire.begin(M5STICKV_SDA_PIN, M5STICKV_SCL_PIN);
    ...
    M5StickVWire.setClock(100000);
    // A bound on how long a stuck bus can hold up whichever task calls into
    // this Wire (PolicyMode's own poll, on core 0) -- the M5StickV's own

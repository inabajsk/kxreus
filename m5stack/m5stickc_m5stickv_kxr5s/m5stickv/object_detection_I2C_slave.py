...
class App:
    def __init__(self):
        ...
        # NOT auto-detected via a second I2C bus (I2C1, scanned for an LCD
        # controller) as this used to be: real-hardware testing (2026.9)
        # found that simply constructing a second machine.I2C object and
        # scanning it -- regardless of what, if anything, answers -- leaves
        # I2C0's own slave mode permanently unable to answer any master ever
        # again (0 acks out of hundreds of probes, reproduced identically
        # across two separate physical UnitV units and two host boards).
        # This file now lives in its own per-board directory
        # (~/kxreus/atom/m5stickv, as opposed to ~/kxreus/atom/m5UnitV), so
        # the board is already known from which copy got flashed -- no
        # runtime detection needed, and this is the one hardcoded per-copy
        ...
        # 空いている別のレジスタ番地を使う。
        #
        # デフォルトは無効(0)。通常は使わない重い処理(色ブロブのクラスタ
        # リングをフレーム毎に実行)であり、他の検出(nn_en/apriltag_en等)と
        # 同じく「明示的にONにされて初めて動く」規約に合わせた(2026.9、
        # 実機確認: 外部I2Cスレーブがホストからの問い合わせに一切応答しなく
        # なる不具合の調査中、この処理がデフォルトで動いていたことが判明)。
        # 使うときはホストから明示的に1を書き込むこと。
        self.ADDR_CUBE_DISCOVERY_ENABLE = 0xFD
        self.i2c_register[self.ADDR_CUBE_DISCOVERY_ENABLE] = 0
    ...
    # 「エイプリルタグ見つけました。(個数)。(1個目のID)番です。(2個目のID)
    # 番です。...」と発話する(2026.9、個数を数える専用クリップは無いため、
    # 個数もspeak_numberで数字として読み上げる点はIDの読み上げと区別が付き
    # にくいが、既存クリップの組み合わせで表現できる範囲での実装)。
    # tag_ids: 今回見つかった全タグのID一覧(1つでも可、重複なし)。
    def speak_apriltag_found(self, tag_ids):
        self._play_clip_path(self.EYE_PREFIX_CLIP)  # 「左目で」/「右目で」
        self._play_clip_path(self.APRILTAG_FOUND_CLIP)
        self.speak_number(len(tag_ids))
    ...
    def main(self):
        ...
        try:
            while(True):
                ...
                # AprilTagを見つけたら「エイプリルタグ見つけました。(個数)。
                # (各ID)番です。...」と、見つかった個数と全IDを発話する
                # (2026.9)。同じ組み合わせ(タグの集合)が続く限り、または
                # 場所が変わらない限り再発話しない -- identを「見つかった
                # 全IDのソート済みタプル」にすることで、集合が変わった時
                # (増減・入れ替わり)だけ_speak_debounce_updateが再トリガー
                # する。位置は全タグの中心の平均を代表位置として使う。
                try:
                    if apriltag_bbox:
                        tag_ids = sorted({b.id() for b in apriltag_bbox})
                        avg_x = sum(b.x() + b.w() / 2.0 for b in apriltag_bbox) / len(apriltag_bbox)
                        avg_y = sum(b.y() + b.h() / 2.0 for b in apriltag_bbox) / len(apriltag_bbox)
                        tag_pos = (avg_x, avg_y)
                        tag_ident = tuple(tag_ids)
                    else:
                        tag_ids = []
                        tag_pos = None
                        tag_ident = None
                    if speak_apriltag_en and self._speak_debounce_update(
                            self.apriltag_speak_state, bool(apriltag_bbox), tag_pos, ident=tag_ident):
                        self.speak_apriltag_found(tag_ids)
                except Exception as e:

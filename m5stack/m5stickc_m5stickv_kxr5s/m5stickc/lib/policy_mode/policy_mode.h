...
public:
    ...
    /// The union of every actor role any of kxr5s's four robots' own
    /// kActors[] table names (see lib/policy/policy.cpp's own kRobots[]).
    /// Not every robot has both: kxrl4t and kxrl6 carry only WALK, and a
    /// RISE/SIT sequence for either (see riseSequence()/sitSequence(),
    /// chosen per the CONFIGURED robot, not compiled in for just one
    /// anymore) never names GETUP for those two. kxra6g carries neither
    /// -- see policy::RobotId's own comment -- and never reaches this at
    /// all (loop()'s own policy::count() == 0 branch returns before the
    /// RUN/HOLD/RISE/SIT switch that would read it). Index into
...
private:
    ...
    /// while; kxrl4t/kxrl6 (and kxrl4d's own SIT -- no trained lie-down
    /// skill either) just ramp to WALK's own home, the same place a
    /// plain `hold` (button click) puts the hand. Defined in
    ...
    /// Gates draw()'s own M5StickV poll (see its own comment) to a slower
    /// cadence than DRAW_INTERVAL_MS -- an I2C transaction to a device that
    /// may not even be there is not worth paying for at the LCD's own
    /// refresh rate.
    uint32_t next_m5stickv_poll_ms_ = 0;
    ...
    ///
    /// WALK: the only actor this build has.
    uint8_t actor_ = WALK;

...
/// wrong id would drive whatever servo actually sits at that slot. kxrl4t's
/// head (2 axes) is unwired for exactly this reason (see
/// tools/export_policy.py's calibration.yaml and lib/policy/policy_spec_
/// kxrl4twalk.h): the standard KXR-L4T leg/arm wiring is confirmed against
/// kxreus/rcb4robotconfig.l across three independent variants, the head
/// wiring is not standardised across those same variants.
...
void buildServoOrder() {
    ...
    // robot with fewer joints than the widest one must stop short of it:
    // policy::servoIds()/homeRad()/etc for the configured robot's own
    // compiled-in actor are exactly actDim() long in flash, and reading
    // past that end is an out-of-bounds read of whatever .rodata happens
    // to sit next.
    const size_t act_dim = policy::actDim();
    for (size_t i = 0; i < act_dim; i++) g_sorted_to_joint[i] = i;
    // Insertion sort: at most 19 elements, once, at startup.
...
// same convention lib/policy/policy.cpp's own kRobots[] follows.
...
void PolicyMode::beginRamp(const float* target, float seconds) {
    ...
    // target is riseSequence()/sitSequence()'s own homeRadOf() call --
    // exactly actDim() long in flash, same reasoning as
    // writeJointTargets()'s own comment above.
    float span = 0.0f;
    for (size_t i = 0; i < policy::actDim(); i++) {
        ramp_to_[i] = target[i];
...
void PolicyMode::loop() {
    ...
    // wiring, so an M5StickC that has never had its robot identity set
    // (see net.cpp's own "robot <name>" serial command, or the button-
    // only ROBOT mode) must not drive a single servo -- stuck at IDLE
    // with servos free, no matter what is asked, the same way
    // net::Status::UNCONFIGURED already refuses to guess a Wi-Fi network

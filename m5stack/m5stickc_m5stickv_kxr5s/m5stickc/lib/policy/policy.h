...
// kxrl4t/kxrl4d/kxrl6's own policy_spec_*.h files (all three trained at
// the same control rate, same stretch, same obs layout up to where the
// per-robot joint count takes over) rather than assumed. Bound to
// kxrl4twalk's own spec arbitrarily -- any one of the three would give
// the same numbers.
...
/// robot's own obs/act dim -- kxr5s carries three robots' worth of actors
...
/// own policy_spec.h constant could be. Headroom kept above kxrl4d's own
/// 68/19 (the largest compiled in here): kxreus's own convention keeps
/// every one of these robots at or under 36 actuators and 96
/// observations, the same bound the AtomS3 kxr5s tree (which also carries
/// kxrl2g, 77/22) uses -- kept equal on purpose so the two trees' own
/// policy_mode.cpp/.h stay identical rather than diverging over a buffer
/// size neither actually needs to be tight.
...
///
/// kxr5s carries every one of kxrl4t/kxrl4d/kxrl6's own trained actors in
/// the SAME flash image (kxrl4d's own two, walk and getup; one each for
/// the other two) rather than the one-tree-per-robot split every earlier
/// build used -- see robotId()'s own comment for how one of them becomes
/// "the" robot this particular unit is. kxrl2g is NOT one of the three:
/// that robot's own body was rewired for an AtomS3's own I/O (see
/// ~/kxreus/m5stack/atoms3_m5stickv_kxr5s), not this board's, so an
/// M5StickC never runs in it.
namespace policy {
...
/// one is that the SAME .bin serves any of the four, and only a
/// lightweight per-unit step (a serial "robot <name>" command, or the
/// button-only ROBOT mode -- see lib/robot_select_mode) says which.
///
/// KXRA6G is unlike the other three: its body is modular (arms and lower
/// body both swap independently), so it has no ONE fixed servo layout to
/// train a walking actor against and carries no actor at all here (see
/// policy.cpp's own kRobots[] row for it, actor_count 0). What it
/// carries instead is its own onboard motion table (see motionCount()/
/// motion()), which PolicyMode's own Request::MOTION already calls
/// through Rcb4Link::callMotion() regardless of whether an actor is
...
/// currently attached, not a policy running on this chip. Same robot,
/// same reasoning as ~/kxreus/m5stack/atoms3_m5stickv_kxr5s's own
/// RobotId -- this board is just another COM adapter for the same RCB-4.
enum class RobotId : uint8_t { KXRL4T, KXRL4D, KXRL6, KXRA6G, COUNT };
...
/// RobotId::COUNT if `name` does not match any of the four -- callers
/// (net.cpp's own serial command) treat that as a rejected request, not a
...
/// wiring, so PolicyMode refuses to run at all (see its own loop() guard)
/// until this is true, the same way net::Status::UNCONFIGURED already
/// refuses to guess a Wi-Fi network.
...
/// This unit's own LCD mounting -- per robot rather than a single
/// compile-time DISPLAY_ROTATION the way a per-robot tree had (today all
/// four of kxrl4t/kxrl4d/kxrl6/kxra6g use 0, but which robot's body an
/// M5StickC ends up mounted in decides this now, not which firmware
/// image it happened to be flashed with -- see the AtomS3 kxr5s tree's
/// own copy of this comment for why that stopped being quite true there
/// once one physical unit's identity became switchable: worth rechecking
/// here too if a differently mounted unit ever turns up).
int displayRotation();
...
/// kxrl4d's own two (walk, getup), one each for the other two. Flash
...
/// own convention (see policy.cpp's kRobots[]) always the walking actor,
...
/// float32: fewer than a hundred numbers is noise next to the tens of
/// thousands of weights+biases below, and they feed the very first
/// subtraction/multiply every observation goes through, where a bad scale
/// would cost more than it saves.
///
/// @return false if this device could not even try: begin()'s own early
///         malloc of the upload buffer failed (see its own comment on why
///         that happens once, at boot, rather than fresh on every call --
///         a fragmented heap can refuse a single allocation this size
///         even with plenty of TOTAL free heap left, which used to make
///         this call fail unpredictably depending on how long the device
///         had been running), or the "uploaded" actor -- the very buffer
///         this would overwrite -- is the one currently selected and
///         running. The caller (net.cpp) turns this straight into an
